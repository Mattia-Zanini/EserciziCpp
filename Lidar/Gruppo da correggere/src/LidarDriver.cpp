...
LidarDriver::LidarDriver(double a)
    : index(0), buffer_size(1), angular_resolution(a) {
  if (a < 0.1 || a > 1)
    throw std::out_of_range("Error: out fo bound exception\n");
  ps = new Scan[DIM_BUFFER];
}

void LidarDriver::new_scan(std::vector<double> &v) {
  if (index < DIM_BUFFER) {
    // elemino le cose in index cosi non creao garbage ne con scan() e neanche
    // quando sovrascrivo
  ...
  if (buffer_size <
      DIM_BUFFER) { // se pieno non serve incrementarla quindi lo faccio a parte
    buffer_size++;
  }
}

// il vettore che restituisce deve essere quello
// inserito più vecchio / che è li da più tempo

std::vector<double> LidarDriver::get_scan() {
  if (index = 0)
    throw EmptyBufferException();
  if (buffer_size < DIM_BUFFER || index == DIM_BUFFER - 1) {
    return ps[0].to_vector();
  } // buffer non ancora riempito
  else {
    return ps[index].to_vector();
  }
}
...
double LidarDriver::get_distance(double degree) {
  ...
  if (index - 1 == -1) {
    ...
    return ps[index - 1].get_reading(degree);
  }
}
